class AlliancesController < ApplicationController
  ...
  def top_list
    @alliances = @alliances.noticeable.desc(:current_member_count, :growth_ratio)
  end

  def growing
    @alliances = @alliances.noticeable.gt(growth_ratio: 1).lt(growth_ratio: 5).desc(:growth_ratio, :current_member_count)
  end

  def collapsing
    @alliances = Alliance.noticeable.ne(predicted_collapse: nil).asc(:predicted_collapse).desc(:current_member_count)
  end

  def landlords
    @alliances = @alliances.noticeable.gt(sov_held: 0).desc(:sov_held, :current_member_count)
  end
