...
(function() {
  jQuery(function() {
    ...
    return $('#top-alliances dt').each(function() {
      var allianceLine, topEntry;
      topEntry = $(this);
      allianceLine = function() {
        return topChart.highcharts().series[topEntry.data('index')].graph;
      };
      topEntry.on('mouseenter', function() {
        return allianceLine().animate({
          'stroke-width': '5'
        }, 'easing');
      });
      return topEntry.on('mouseleave', function() {
        return allianceLine().animate({
          'stroke-width': '2'
        }, 'easing');
      });
...
(function() {
  jQuery(function() {
    ...
    form = $('#main-search');
    if (form.length > 0) {
      form.find('.input-group-addon').on('click', function() {
        return form[0].submit();
      });
      return form.find('input').autocomplete({
        source: '/alliances',
        select: function(e, ui) {
          if (ui.item) {
            return window.location = "/alliances/" + ui.item.value;
          }
        }
      });
    }
